...
public class RestHandler extends AsyncTask<Object, Object, String> {
	...
	//updates the thread so we dont have to make a new thread;
	public void update(String movie, Boolean newSearch) {
		this._updateMovie = getBetterTitle(movie);
		try {
			Final_URL = new URI("http://www.omdbapi.com/?t=" + _updateMovie);
		} catch (URISyntaxException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		this._url = this.Final_URL;
	}
	public void setURL(URI better){
		this._url = better;
	}
	...
	private String getBetterTitle(String _movie) {
		...
		Log.v("in getBetterString, I got", finalInput);
	...
	protected String doInBackground(Object... arg0) {
		...
		Log.v("I am in the back ground with:", text);
		return text;
	}
	...
	public void makeAnArray(String toParse) {
		...
		try {
			...
			for (int i = 0; i < 5; i++) {
				...
				titles[i] = jObject.getString("Title");
		...
		Log.v("in RestHandler.makeAnArray", "with");
		for(int i = 0; i < arr.length; i++){
			Log.v("Number " + (i+1), arr[i] );
		}
		_tm.onArrayMade(arr,titles);
	}
...
//			image_url = toBeHandled.getString("Poster");
			...
			Log.v("I was given: ", toParse);
			e.printStackTrace();
		}// end
		
		//create and send movie to handler
		_mm.onMovieMade(new Movie(title, release_date, year, mpaa_rating,
				genre, cast, plot, runtime));
	}
	...
	public interface TitleMaker {
		public void onArrayMade(String[] arr,String[] titles);
	}
